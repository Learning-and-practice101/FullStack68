...
[lecture](https://fullstack-68.github.io/lectures/src/D01_database_design/D01.html)<br>

### quick note:
```
Edger F.codd -> คนสร้าง database คนแรกๆ

SQlite database ไม่ใหญ่มาก

postgres เกิดช่วงปี 80

ranking :relational ก็มีคนใช้ระดับต้นๆ
...
```
### AICD
<img width="661" height="863" alt="image" src="https://github.com/user-attachments/assets/319519e7-f428-488c-8ce7-8a352cd3a415" /><br>
```
🅰️ A - Atomicity (ความเป็นอะตอม)
หมายถึง “ทั้งหมดหรือไม่มีเลย”

ถ้าเราสั่งทำหลายๆ ขั้นตอนในธุรกรรม (transaction) เช่น เพิ่มข้อมูลหลายแถวในฐานข้อมูล

ถ้า ทุกขั้นตอนสำเร็จ → ค่อยบันทึกข้อมูลลงจริง

ถ้ามีขั้นตอนใด ล้มเหลว → ยกเลิกทั้งหมดเหมือนไม่เคยเกิดขึ้น

ตัวอย่างง่ายๆ: เหมือนกดสั่งซื้อสินค้าในแอป ถ้าชำระเงินไม่ผ่าน ก็จะไม่มีรายการสั่งซื้อเกิดขึ้นเลย

🅲 C - Consistency (ความถูกต้องสม่ำเสมอ)
ฐานข้อมูลต้อง ถูกต้องเสมอ ก่อนและหลังจากทำธุรกรรม

เช่น กฎบางอย่างของระบบต้องไม่ผิด เช่น ยอดเงินรวมของบัญชีห้ามหายไปหรือเพิ่มขึ้นผิดปกติ

ตัวอย่างง่ายๆ: โอนเงินจากบัญชี A ไป B ต้องหักจาก A และเพิ่มให้ B อย่างถูกต้อง

🅸 I - Isolation (ความเป็นอิสระ)
ธุรกรรมที่ทำพร้อมกัน ต้องไม่รบกวนกัน

เช่น ถ้าคนสองคนซื้อของพร้อมกัน ระบบต้องจัดการให้ไม่เกิดข้อมูลปนกัน หรืออ่านข้อมูลผิด

ตัวอย่างง่ายๆ: สองคนสั่งซื้อสินค้าชิ้นสุดท้ายพร้อมกัน ต้องมีระบบแยกให้คนใดคนหนึ่งได้ไปจริงๆ

🅳 D - Durability (ความคงทน)
เมื่อข้อมูลถูกบันทึกแล้ว จะ ไม่หาย แม้ระบบจะล่ม ไฟดับ หรือมีปัญหา

เพราะระบบจะบันทึกสำรองไว้ เช่น replicate ข้อมูลไปยังหลายที่

ตัวอย่างง่ายๆ: ถ้าเรากด "บันทึก" แล้วเครื่องดับ ข้อมูลจะยังอยู่เมื่อเปิดกลับมา
```

### NoSQL <br>
```
🔹 NoSQL คืออะไร?
ย่อมาจาก "Not Only SQL" (ไม่จำเป็นต้องใช้ SQL อย่างเดียว)

ใช้เก็บข้อมูลในรูปแบบที่ไม่ใช่ตารางแบบดั้งเดิม (relational table)

ออกแบบมาเพื่อ รองรับข้อมูลปริมาณมาก, ขยายระบบง่าย, และ ทำงานได้แม้ระบบบางส่วนล่ม
(เน้นที่ scalability และ availability)

🔹 ประเภทของ NoSQL
Document-oriented
เก็บข้อมูลแบบเอกสาร เช่น JSON หรือ XML
👉 เหมาะกับข้อมูลที่มีโครงสร้างยืดหยุ่น

Column-oriented
เก็บข้อมูลแบบแยกตาม column
👉 ใช้ในระบบที่เน้นการวิเคราะห์ข้อมูลจำนวนมาก เช่น Big Data

Graph-based
เหมาะกับข้อมูลที่มีความสัมพันธ์ซับซ้อน เช่น โซเชียลเน็ตเวิร์ก
👉 เช่น Neo4j

Key-Value pair
เก็บข้อมูลแบบ key กับ value เช่น Dictionary
👉 รวดเร็ว เรียบง่าย เช่น Redis

Time series
เหมาะกับข้อมูลตามช่วงเวลา เช่น ข้อมูล sensor หรือ log
👉 เช่น InfluxDB

รูปแบบข้อมูลที่ใช้บ่อย: JSON, BSON (binary of json), XML
```
